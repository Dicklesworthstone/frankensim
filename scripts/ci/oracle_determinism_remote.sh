...
# frankensim-uf7cw). Runs on a worker whose DET_DIR/snapshot was assembled
# and streamed from the AUTHORITATIVE checkout (mirrors carry no .git, so
# archive-based freezing happens on the origin host).
#
# Contract (env):
...
#
# The EXIT trap self-reports the terminal status. BOTH observation steps
# (goldens, probe) are failure-guarded so receipts ALWAYS reach stdout even
# on red legs — a failing witness is data, not silence.
# Exit codes: 2 usage; 3 refusal; 1 goldens or probe red.
set -euo pipefail
...
GOLDENS_PASS=true
if ! cargo test -p fs-query --test bore -- gb_003 gb_004 2>&1 |
    tail -200 >"$DET_DIR/bore_goldens.log"; then
    GOLDENS_PASS=false
fi

PROBE_PASS=true
if ! FS_DET_ENV="$LABEL" cargo run -q -p fs-query --example determinism_probe \
    >"$DET_DIR/probe.jsonl"; then
    PROBE_PASS=false
fi
...
{"record":"leg_receipt","schema":"frankensim-oracle-leg-v1","mode":"$MODE",
 ...
 "goldens_pass":$GOLDENS_PASS,"probe_pass":$PROBE_PASS,
...
if [ "$GOLDENS_PASS" != "true" ] || [ "$PROBE_PASS" != "true" ]; then
    echo "leg failed: goldens/probe red inside frozen tree; receipts emitted above" >&2
    exit 1
fi
