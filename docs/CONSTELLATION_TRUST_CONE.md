...
| `frankensqlite` | active | 2 | 1 | 28 | 36 | critical | high | P1 |
...
- **Measured usage:** `active`; runtime consumers: fs-ledger, fs-vskeleton; dev consumers: fs-ledger; production/test references: 28/36
- **Sampled API references:**
  - `fsqlite` at `crates/fs-ledger/src/crosswalk.rs`:14
  - `fsqlite` at `crates/fs-ledger/src/identity_migration.rs`:24
  - `fsqlite` at `crates/fs-ledger/src/lib.rs`:92
  - `fsqlite` at `crates/fs-ledger/src/lib.rs`:2772
  - `fsqlite` at `crates/fs-ledger/src/lib.rs`:2798
