...
| `check-spine-metrics` | a missing, corrupt, or internally inconsistent `spine-metrics.json` beads snapshot (a gate that reports zero findings on a rotted input is how gates die), or an e2e status outside the admitted v1 state (bead `frankensim-o5et9`). A tracker that moved since the snapshot renders a visible `stale-snapshot` policy note — never a silent pass, never a wedge; regenerate deliberately with `generate-spine-metrics`. The snapshot feeds the dashboard's blocked-ratio/actionable rows so the live beads store never byte-destabilizes `check-program-metrics` |
