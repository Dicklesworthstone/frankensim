...
Snapshot: **2026-09-10 UTC**, refreshed after the warm NASA 316 thermal table and preceding material additions, current shared working tree on main, including uncommitted material additions. This is an inventory of populated source records, not a claim that every bundle compiles, every property is measured, or every material can run a complete simulation.
...
The sourced catalog lives in **[data/matdb/seed-v1](../data/matdb/seed-v1/README.md)** as static text files. It currently occupies **0.878114 MB** (0.837435 MiB), including its README and license notice. The TSV data and manifests alone occupy **0.752974 MB**. There are **183 source bundles**, including **165 bulk-material bundles**.

The `aluminum-2024-t3-first-heating-km2024` bundle adds three thermal curves
with nine values at 25/50/100 °C, restricted to first heating. Cp is calculated,
warm density is derived, and conductivity uses measured diffusivity with those
inputs; these are not nine independent measurements. Two runs of the prebuilt
remote compiler admitted all three claims and produced identical 10,852-byte
packs (log `/tmp/greenosprey-m03-km2024-prebuilt.log`). Its source/store/reopen/
enthalpy/heating regression now passes required-remote on vmi1152480: one test,
zero failures or ignored tests, including three mass/power trajectories, an
independent temperature reference, energy, replay and domain refusal/retry.
Log: `/tmp/frankensim-m03-stored-heating-retry-20260911.log`. No 20–25 °C,
elastic coverage, aging kinetics or experimental qualification is added.

The new `stainless-316-nasa-tp216435` bundle retains three thermal curves
(density, specific heat and conductivity), with nine literal Table 23 values
at 273, 300 and 400 K. Intermediate values use declared linear interpolation.
Its generic 316 designation, unknown UNS/heat treatment/product form/pressure
and unstated uncertainty remain explicit; it cannot silently complete a
different 316 or 316L specimen. The [warm-316 discovery request](../examples/material-discovery/warm-316-thermal.json)
selects this thermal-only profile. Its real store/reopen/query and 300–400 K
conduction regression passed on remote worker vmi1152480 on 2026-09-10:
one test passed, zero failed or ignored. Independent field/flux checks,
deterministic replay, source-context refusals and discovery passed; relative
energy closure was 1.3703324189659064e-14. The log is
`/tmp/greenosprey-m03-warm-316.log`. Scalar isotropic transport
is a declared calculation approximation, not measured directional evidence.
This addition does not supply the full M03 two-metal transient thermoelastic profile.
...
The Pilkington soda-lime float and SCHOTT BOROFLOAT 33 profiles are explicit
opt-in frozen engineering references with six scalar inputs each. Their
author-selected query domain is 298.15–299.15 K for a 25–26 °C frozen
calculation; it is not source coverage, measurement accuracy, or a claim that
the six values were co-measured. Each claim retains its source temperature (or
unknown temperature) in the observation, requires
`engineering-reference-model=1` and `source-temperatures-matched=0`, and keeps
the printed CTE as a mean coefficient over its source interval. No
instantaneous expansion law is supplied. The focused remote integration test
passed on 2026-09-09: both profiles reached the existing gated lumped heat
operator through the compiler, reopened store, discovery and material resolver.
For the prescribed conductance model, their 100 s temperature rises were
0.346824281 K and 0.477083374 K; independent exponential-response, energy,
deterministic replay and unsupported-context checks passed. This verifies the
frozen reference calculation, not experimental glass behavior.
...
The water-vapor parameter pack now joins that dry-air source through a
receipt-preserving humid-air adapter. Five humid states, including both
temperature endpoints and the saturation limit, pass independent mixture,
cylinder-drag and classical sound-absorption references through the real
compiler, reopened store and separate component discovery. The existing gas
constructor now mixes viscosity with Wilke's rule and conductivity with WMS,
alongside consistent ideal-mixture heat capacity and sound speed. Exact dry
recovery, humidity-basis conversion, replay and unsupported-state refusals pass.
The sourced range is 273.15–313.15 K and 80–110 kPa. Constant vapor heat
capacity, ambient Sutherland extrapolation and Eucken conductivity remain
explicit engineering approximations; condensation, real-gas corrections and
experimental accuracy are not qualified.
...
The DOWTHERM SR-1 and DOWFROST coolant packs each supply density, specific heat,
conductivity and dynamic viscosity at five temperatures from 40 to 60 °C,
at exactly 50% glycol by volume. The [liquid adapter](../crates/fs-material/src/liquid.rs)
preserves the four source receipts and derives kinematic viscosity, thermal
diffusivity and Prandtl number. The actual compiler, reopened store and discovery
path feed existing lumped heat and LBM channel solvers at every knot and one
interpolated temperature. The eleven-case source runner passed remotely on
2026-09-09 UTC (receipt `frankensim-material-sources.AlDYg9`). Maximum channel
profile error was 1.31% of the independent analytic peak; heat energy closure
error was below 9e-14 J. These are numerical checks of a fixed-property heat
approximation and steady laminar flow. Source pressure and concentration-reference
...
Ensinger TECAFINE PE300 natural now contributes four 2017 product-sheet
comparison facts for named heat-stabilized natural PE-HD stock shapes: density,
tensile modulus, tensile yield strength and tensile yield strain. The source
does not state test temperature, conditioning, rate, direction, stock
dimensions or production history. Temperature, process and rate have typed
unknown-condition flags; the remaining omissions are retained in source notes. Its
printed CLTE and service-temperature band are not thermal-property claims.
This partial bundle does not complete P01 or reach a thermal consumer.

MCAM Proteus Homopolymer PP Natural adds six producer comparison facts from
its 2023 datasheet. Conductivity is 0.22 W/(m K) at exactly 23 °C; tensile
modulus retains the stated 1 mm/min test speed. Density has no stated source
temperature, and expansion is a mean over 23–100 °C. Heat capacity and Poisson
ratio are absent. The new `slab_from_card` consumer computes reference-state
...
[product page](https://www.iplex.com.au/products/pvc-pressure-system/pvc-u):
density, tensile modulus, Poisson ratio, heat capacity and conductivity.
Density is an explicitly authored conversion of specific gravity 1.47 using
1000 kg/m³ reference-water density. The source identifies extrusion, but leaves
the exact compound, extrusion schedule and property test conditions unspecified.
...
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| [stainless-316-nasa-tp216435](../data/matdb/seed-v1/stainless-316-nasa-tp216435/manifest.tsv) | 3 | 0 | 3 | 9 | 5.24 | `density` [kg/m3]; `specific-heat-capacity` [J/kg/K]; `thermal-conductivity` [W/m/K]; generic 316, linear table interpolation at 273–400 K; exact UNS/form/heat/pressure unknown |
| [aluminum-2024-t3-first-heating-km2024](../data/matdb/seed-v1/aluminum-2024-t3-first-heating-km2024/manifest.tsv) | 3 | 0 | 3 | 9 | 3.39 | `density` [kg/m3]; `specific-heat-capacity` [J/kg/K]; `thermal-conductivity` [W/m/K]; T3 sheet first heating, 298.15–373.15 K; source calculated/derived thermal inputs, pressure unknown |
| [stainless-14401-thyssenkrupp-2017](../data/matdb/seed-v1/stainless-14401-thyssenkrupp-2017/manifest.tsv) | 2 | 5 | 1 | 4 | 4.53 | `young-modulus` [GPa], 293.15–773.15 K linear reference curve; `mean-linear-expansion-coefficient-from-20c` [K^-1], five distinct reference/end temperature intervals; not instantaneous alpha |
| [roechling-polystone-g-natural-reference](../data/matdb/seed-v1/roechling-polystone-g-natural-reference/manifest.tsv) | 3 | 3 | 0 | 0 | 5.40 | `density` [kg/m3]; `specific-heat-capacity` [J/kg/K]; `thermal-conductivity` [W/m/K]; named HDPE reference, explicit frozen 25–26 °C model opt-in; source temperatures, product form and process unknown; no scalar expansion or elastic law |
...
| [pilkington-float-glass-reference](../data/matdb/seed-v1/pilkington-float-glass-reference/manifest.tsv) | 6 | 6 | 0 | 0 | 6.98 | `density` [kg/m3]; `young-modulus` [Pa]; `poisson-ratio` [1]; `specific-heat-capacity` [J/kg/K]; `thermal-conductivity` [W/m/K]; `mean-linear-thermal-expansion-coefficient` [K^-1]; opt-in 25–26 °C frozen reference for generic soda-lime float glass, source temperatures differ or are unstated, mean expansion only |
| [schott-borofloat33-reference](../data/matdb/seed-v1/schott-borofloat33-reference/manifest.tsv) | 6 | 6 | 0 | 0 | 6.51 | `density` [kg/m3]; `young-modulus` [Pa]; `poisson-ratio` [1]; `specific-heat-capacity` [J/kg/K]; `thermal-conductivity` [W/m/K]; `mean-linear-thermal-expansion-coefficient` [K^-1]; opt-in 25–26 °C frozen reference for annealed BOROFLOAT 33, source temperatures differ, mean expansion only |
