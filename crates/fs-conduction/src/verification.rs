...
//!
//! This adapter consumes [`ConductionProblem`] without inventing
...
pub type MeanTemperatureSolution =
    fs_verify::conduction::MeanTemperatureSolution<ConductionSolution>;
...
pub type MeanFieldBound = fs_verify::conduction::MeanFieldBound<ConductionSolution>;
...
fn admit(cx: &Cx<'_>, problem: ConductionProblem<'_>, budget: FluxBudget) -> Result<Admitted> {
    ...
    if ne > budget.max_cells
        || n > budget.max_cells.saturating_mul(4)
    ...
    for (e, tet) in mesh.complex().tets.iter().enumerate() {
        ...
        if model.is_temperature_dependent() {
            return Err(unsupported(
                "temperature-dependent conductivity needs a nonlinear residual bound",
            ));
        }
        if !matches!(model.temperature_span(), TemperatureSpan::Unbounded) {
            return Err(unsupported(
                "bounded material validity needs a separate continuum temperature-range proof",
            ));
        }
        ...
        let condition = match problem.boundary.condition_for(slot) {
            ...
            Some(ThermalBc::Neumann { outward_flux }) => {
                ...
                if flux.iter().any(|&v| v != flux[0]) {
                    return Err(unsupported(
                        "nonconstant face Neumann flux requires a higher-order normal trace",
                    ));
                }
            ...
            Some(ThermalBc::Robin { htc, t_ref }) => {
                ...
                if h.iter().any(|&v| v != h[0]) {
                    return Err(unsupported(
                        "nonconstant face Robin coefficient cannot be replaced by its mean",
                    ));
                }
                BoundaryCondition::Robin {
                    h: h[0],
                    reference: vertices.map(|v| t_ref.at(v)),
                }
            }
    ...
    admitted
        .problem(mesh.positions())
        .validate_conductivity(budget, || cx.checkpoint().is_ok())?;
    Ok(admitted)
}
...
fn dual_boundary(cx: &Cx<'_>, problem: ConductionProblem<'_>) -> Result<ThermalBoundary> {
    ...
    for (slot, face) in problem.mesh.boundary().iter().enumerate() {
        poll(cx)?;
        if let Some(region) = problem.boundary.region_for(slot) {
            faces[region].insert(face.face);
        }
    }
    ...
    for (region, condition) in problem.boundary.conditions().iter().enumerate() {
        ...
        let dual = match condition {
            ...
            ThermalBc::Robin { htc, .. } => ThermalBc::Robin {
                htc: htc.clone(),
                t_ref: ScalarField::Uniform(0.0),
            },
        };
        builder = builder.region(
            &problem.boundary.region_names()[region],
            |face| faces[region].contains(&face.face),
            dual,
        )?;
    }
    ...
    if dual.adiabatic_remainder_faces() != problem.boundary.adiabatic_remainder_faces() {
        return Err(
            TetError::Invalid("dual boundary partition does not reproduce the primal").into(),
        );
    }
    Ok(dual)
}
...
fn bound_field(
    ...
    for &(vertex, value) in problem.boundary.dirichlet() {
        ...
        if temperature.get(vertex) != Some(&value) {
            return Err(
                TetError::Invalid("candidate does not match prescribed Dirichlet trace").into(),
            );
        }
    }
