...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let (n, source) = match args.as_slice() {
        [] => (4, 2.0),
        [n, source] => (n.parse::<usize>()?, source.parse::<f64>()?),
        _ => return Err("usage: conduction_mean [cells-per-axis source-W-per-m3]".into()),
    };
    if ![2, 4, 8, 16].contains(&n) || !source.is_finite() || !(0.0..=1_000.0).contains(&source) {
        return Err("admitted example: n in {2,4,8,16}, source in [0,1000] W/m^3".into());
    }
    let (complex, vertices) = unit_cube(n);
    let mesh = ConductionMesh::new(complex, vertices)?;
    let boundary = ThermalBoundaryBuilder::new(&mesh)
        .region("left", |f| on_box_face(f.centroid[0], 0.0), ThermalBc::dirichlet(300.0)?)?
        .region("right", |f| on_box_face(f.centroid[0], 1.0), ThermalBc::dirichlet(300.0)?)?
        .adiabatic_remainder().finish()?;
    let material = ConductivityModel::isotropic_declared(1.0)?;
    let forcing = ScalarField::Uniform(source);
    let mut config = MeanSolveConfig::default();
    config.primal.nonlinearity = Nonlinearity::FixedPoint { relaxation: 1.0, max_backtracks: 4 };
    ...
    let result = ArenaPool::new(ArenaConfig::default()).scope(|arena| {
        ...
        solve_with_mean_bound(&cx, ConductionProblem { mesh: &mesh, boundary: &boundary,
            material: &material, element_materials: None, source: &forcing }, config)
    })?;
    let exact = 300.0 + source/12.0;
    let bound = &result.bound;
    println!(concat!(
        "{{\"functional\":\"whole-domain-volume-mean-temperature\",\"unit\":\"K\",",
        "\"cells\":{},\"source_w_m3\":{:.17e},\"mean_lower\":{:.17e},\"mean_upper\":{:.17e},",
    ...
    ), mesh.element_count(), source, bound.enclosure.lo, bound.enclosure.hi,
        bound.candidate_mean.lo, bound.candidate_mean.hi,
        bound.integral.primal.energy_error_upper, bound.integral.dual.energy_error_upper,
        exact, bound.enclosure.lo <= exact && exact <= bound.enclosure.hi);
