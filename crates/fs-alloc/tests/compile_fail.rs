...
//! (frankensim-epic-bedrock-6ys.21.1.3.1 and .2, acceptance criteria 4):
//! every misuse clause must be rejected by the compiler itself — no runtime
//! fallback that could silently accept a cloned owner, a reissued charge,
//! a use-after-close, an escaped parent lifetime, or a fabricated receipt.
//!
...
//! refuses unbounded-root configuration; delegation refuses unbounded
//! parents with `unbounded_parent`), asserted by deterministic refusal codes.
...
const CASES: &[(&str, &str, &str)] = &[
    ...
    (
        "fabricated_published_guard_literal",
        concat!(
            "use fs_alloc::{OperationMemoryLease, PublishedAllocation};\n",
            "let forged = PublishedAllocation {\n",
            "    root: OperationMemoryLease::bounded(8),\n",
            "};\n",
            "let _ = forged;\n",
        ),
        "private field",
    ...
    (
        "guard_raw_value_extraction_absent",
        concat!(
            "fn extract<T>(published: &fs_alloc::PublishedAllocation<T>) -> &T {\n",
            "    published.observe()\n",
            "}\n",
            "fn steal<T>(published: fs_alloc::PublishedAllocation<T>) -> T {\n",
            "    published.into_raw_value()\n",
            "}\n",
            "let _ = extract::<u64>;\n",
            "let _ = steal::<u64>;\n",
        ),
        "no method named `into_raw_value`",
    ),
    (
        "guard_mutable_access_absent",
        concat!(
            "fn poke<T>(published: &mut fs_alloc::PublishedAllocation<T>) {\n",
            "    published.observe_mut();\n",
            "}\n",
            "let _ = poke::<u64>;\n",
        ),
        "no method named `observe_mut`",
    ),
];
