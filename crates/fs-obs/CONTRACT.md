...
- The scanner inventories the repository-owned `crates/`, `tools/`, and
  ...
  are size-checked and reader-bounded before allocation. The scanner does not
  ...
  subset, including Cargo-equivalent quoted and dotted TOML keys; unsupported
  inline package/lib tables refuse rather than disappear. Source count/bytes,
...
- From each admitted library root, ordinary `mod foo;` and direct `#[path]`
  ...
  excluded in either attribute order, and an iterative active/done traversal
  preserves cycle/alias diagnostics without ancestry cloning. Allowances
  require one production owner and ordered absolute invocations; protected
  imports/declarations/renames, legacy or selective `macro_use`, potentially
  external glob imports, and direct or conditional `no_std`/`no_core` cannot
  lend them authority. `crate`/`self`/`super` globs are accepted because they
  cannot alter the explicitly absolute compiler-provided `::std` path; reachable
  protected declarations and invocations are still scanned independently. The
