...
use fs_conduction::verification::{
    ...
    bound_temperature_mean, solve_with_mean_bound,
};
use fs_conduction::{
...
fn config() -> MeanSolveConfig {
    ...
    primal.nonlinearity = Nonlinearity::FixedPoint {
        relaxation: 1.0,
        max_backtracks: 4,
    };
    ...
    ThermalBoundaryBuilder::new(mesh)
        .region(
            ...
            |f| {
                f.vertices
                    .iter()
                    .all(|&v| mesh.positions()[v as usize][0] == 0.0)
            },
        ...
        .region(
            ...
            |f| {
                f.vertices
                    .iter()
                    .all(|&v| mesh.positions()[v as usize][0] == 1.0)
            },
...
fn contains(lo: f64, hi: f64, truth: f64) {
    assert!(lo <= truth && truth <= hi, "[{lo}, {hi}] excludes {truth}");
}
...
fn real_off_diagonal_neumann_solve_and_mean_follow_the_rotated_material_frame() {
    with_cx(|cx| {
        ...
        for axes in [[0, 1, 2], [2, 0, 1]] {
            ...
            let k: [[f64; 3]; 3] = [[2.0, 1.0, 0.0], [1.0, 3.0, 0.0], [0.0, 0.0, 4.0]];
            let k = axes.map(|i| axes.map(|j| k[i][j]));
            let material = ConductivityModel::constant_tensor(k).unwrap();
            let boundary = ThermalBoundaryBuilder::new(&mesh)
                .region(
                    "left",
                    |f| f.vertices.iter().all(|&v| original[v as usize][0] == 0.0),
                    ThermalBc::dirichlet(300.0).unwrap(),
                )
                .unwrap()
                .region(
                    "right",
                    |f| f.vertices.iter().all(|&v| original[v as usize][0] == 1.0),
                    ThermalBc::dirichlet(301.0).unwrap(),
                )
                .unwrap()
                .region(
                    "y-in",
                    |f| f.vertices.iter().all(|&v| original[v as usize][1] == 0.0),
                    ThermalBc::neumann(1.0).unwrap(),
                )
                .unwrap()
                .region(
                    "y-out",
                    |f| f.vertices.iter().all(|&v| original[v as usize][1] == 1.0),
                    ThermalBc::neumann(-1.0).unwrap(),
                )
                .unwrap()
                .adiabatic_remainder()
            ...
            .unwrap();
            contains(result.bound.enclosure.lo, result.bound.enclosure.hi, 300.5);
...
fn heterogeneous_tensors_preserve_the_series_interface_not_the_fallback() {
    with_cx(|cx| {
        ...
        let table = MaterialTable::new([
            (
                MaterialId(1),
                ConductivityModel::constant_tensor([
                    [1.0, 0.0, 0.0],
                    [0.0, 8.0, 1.0],
                    [0.0, 1.0, 3.0],
                ])
        ...
        .unwrap();
        contains(result.bound.enclosure.lo, result.bound.enclosure.hi, 300.65);
        assert!(result.bound.enclosure.hi - result.bound.enclosure.lo < 1e-6);
...
fn existing_field_path_reuses_the_primal_and_matches_the_solved_bound() {
    with_cx(|cx| {
        ...
        let material =
            ConductivityModel::constant_tensor([[4.0, 0.0, 0.0], [0.0, 2.0, 0.0], [0.0, 0.0, 1.0]])
                .unwrap();
        ...
        let reused = bound_temperature_mean(
            cx,
            problem,
            &candidate,
            config().dual,
        ...
        contains(
            reused.bound.enclosure.lo,
            reused.bound.enclosure.hi,
            300.0 + 1.0 / 6.0,
        );
    });
}
...
fn unfinished_primal_keeps_its_algebraic_error_in_the_mean_bound() {
    with_cx(|cx| {
        ...
        let result = bound_temperature_mean(
            cx,
            ConductionProblem {
                mesh: &mesh,
                boundary: &boundary,
        ...
        .unwrap();
        contains(
            result.bound.enclosure.lo,
            result.bound.enclosure.hi,
            300.0 + 1.0 / 6.0,
        );
        contains(
            result.bound.candidate_mean.lo,
            result.bound.candidate_mean.hi,
            300.0,
        );
...
fn malformed_existing_fields_refuse_before_the_dual_solver() {
    with_cx(|cx| {
        ...
        not_finite[0] = f64::NAN;
        let mut wrong_trace = valid.clone();
        wrong_trace[0] = 299.0;
        for candidate in [
            vec![300.0; mesh.vertex_count() - 1],
            not_finite,
            wrong_trace,
        ] {
            ...
            assert!(matches!(
                bound_temperature_mean(cx, problem, &candidate, dual, FluxBudget::default()),
