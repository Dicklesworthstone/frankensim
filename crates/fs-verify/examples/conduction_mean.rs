...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let (n, source, k) = match args.as_slice() {
        ...
        [n, source, kx, ky, kz] => (n.parse::<usize>()?, source.parse::<f64>()?,
            [kx.parse::<f64>()?, ky.parse::<f64>()?, kz.parse::<f64>()?]),
        _ => return Err("usage: conduction_mean [cells-per-axis source-W-per-m3 [kx ky kz]]".into()),
    };
    if ![2, 4, 8, 16].contains(&n) || !source.is_finite() || !(0.0..=1_000.0).contains(&source)
        || k.iter().any(|v| !v.is_finite() || !(0.01..=1_000.0).contains(v)) {
    ...
    let material = ConductivityModel::constant_tensor([[k[0],0.0,0.0],[0.0,k[1],0.0],[0.0,0.0,k[2]]])?;
    ...
    ), mesh.element_count(), source, k, bound.enclosure.lo, bound.enclosure.hi,
