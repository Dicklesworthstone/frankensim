...
impl CoupledFixture {
    fn coupling_at(&self, t: f64) -> f64 {
        let c = (self.coupling)(t);
        assert!(c.is_finite(), "coupling strength must be finite");
        c
    }
    ...
    pub fn monolithic(&self, state: [f64; 2], t0: f64, t1: f64, fine: usize) -> [f64; 2] {
        assert_time_span(t0, t1);
        assert!(fine > 0, "monolithic reference needs at least one substep");
        assert_state(state);
        let mut u = state;
        #[allow(clippy::cast_precision_loss)]
        let dt = (t1 - t0) / fine as f64;
        let f = |t: f64, u: [f64; 2]| -> [f64; 2] {
            let c = self.coupling_at(t);
    ...
    pub fn split_step(&self, state: [f64; 2], t0: f64, t1: f64, subcycles: usize) -> [f64; 2] {
        assert_time_span(t0, t1);
        assert!(subcycles > 0, "split step needs at least one subcycle");
        assert_state(state);
        let mut u = state;
        #[allow(clippy::cast_precision_loss)]
        ...
        for k in 0..subcycles {
            ...
            let c = self.coupling_at(ta + dt / 2.0);
            // Field x: x′ = −x + c·y_frozen (exact 1-D solve).
            let y_frozen = u[1];
            let x_new = (u[0] - c * y_frozen) * fs_math::det::exp(-dt) + c * y_frozen;
            // Field y: y′ = −2y + c·x_frozen (x handed across).
            let x_frozen = x_new;
            let y_new =
                (u[1] - c * x_frozen / 2.0) * fs_math::det::exp(-2.0 * dt) + c * x_frozen / 2.0;
            u = [x_new, y_new];
        }
        u
    }
...
fn assert_time_span(t0: f64, t1: f64) {
    assert!(
        t0.is_finite() && t1.is_finite() && t1 >= t0,
        "time slab endpoints must be finite and ordered"
    );
}

fn assert_state(state: [f64; 2]) {
    assert!(
        state[0].is_finite() && state[1].is_finite(),
        "time-slab state must be finite"
    );
}

fn norm2(a: [f64; 2], b: [f64; 2]) -> f64 {
    let dx = a[0] - b[0];
    let dy = a[1] - b[1];
    fs_math::det::sqrt(dx * dx + dy * dy)
}

fn assert_ledger_entries(entries: &[SlabEntry]) {
    assert!(
        entries.iter().all(|e| {
            e.t0.is_finite()
                && e.t1.is_finite()
        ...
        "slab ledger entries must be finite, ordered, and non-negative"
    );
}
...
impl SlabLedger {
    ...
    pub fn total_defect(&self) -> f64 {
        assert_ledger_entries(&self.entries);
...
pub fn march_instrumented(
    ...
    assert!(
        ...
        "march horizon must be finite and non-negative"
    );
    assert!(slabs > 0, "instrumented march needs at least one slab");
    assert!(
        subcycles > 0,
        "instrumented march needs at least one subcycle per slab"
    );
    assert_state(state0);
    let mut u = state0;
...
pub fn march_adaptive(
    ...
    assert!(
        t_end.is_finite() && t_end >= 0.0,
        "adaptive march horizon must be finite and non-negative"
    );
    assert!(slabs > 0, "adaptive march needs at least one slab");
    assert!(
...
pub fn activation_report(ledger: &SlabLedger, total_error_budget: f64) -> (f64, Activation) {
    assert!(
        total_error_budget.is_finite() && total_error_budget > 0.0,
        "total error budget must be finite and positive"
    );
    let fraction = ledger.total_defect() / total_error_budget;
