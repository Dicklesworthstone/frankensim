...
use fs_topopt::{
    DensityElasticity, DensityFilter, DesignPipeline, MultiLoadOcOptions,
    MultiLoadOcReport, MultiLoadOcTermination, SimpParams, multi_load_optimality_criteria,
};
...
const HELP: &str = "Usage: elastic_topology --output NEW_DIRECTORY [options]
  ...
  --seconds S        Optional wall-time stop; polled BETWEEN component solves
...
fn parse(args: impl IntoIterator<Item = String>) -> Result<Option<Config>, String> {
    ...
    if !(1..=8).contains(&config.cells) || config.iterations > 200 {
        return Err("resource admission requires 1..8 subdivisions and at most 200 updates".into());
    }
...
fn physical_volume(pipeline: &DesignPipeline, rho: &[f64], volumes: &[f64]) -> f64 {
    let total: f64 = volumes.iter().sum();
    pipeline.forward(rho).1.iter().zip(volumes).map(|(r, v)| r * (v / total)).sum()
}

fn uniform_baseline(pipeline: &DesignPipeline, volumes: &[f64], cap: f64) -> Vec<f64> {
    ...
    assert!(physical_volume(pipeline, &lower, volumes) <= cap, "density floor exceeds volume cap");
    let (mut lo, mut hi) = (1e-3, 1.0);
    for _ in 0..56 {
        let mid = 0.5 * (lo + hi);
        let candidate = vec![mid; volumes.len()];
        if physical_volume(pipeline, &candidate, volumes) <= cap {
            lo = mid;
            lower = candidate;
        } else { hi = mid; }
    }
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let rho0 = uniform_baseline(&pipeline, &volumes, config.volume);
    let options = MultiLoadOcOptions {
        volume_fraction: config.volume, max_iterations: config.iterations,
        ..MultiLoadOcOptions::default()
    };
    // The optional time budget covers optimization, not mesh/filter setup or
    // output I/O. A stop exports retained fields without another physics solve.
    let start = Instant::now();
    let report = multi_load_optimality_criteria(&pipeline, &mut elasticity, &[
        LoadCase { force: &fy, weight: config.weight_y },
        LoadCase { force: &fz, weight: 1.0 - config.weight_y },
    ], &rho0, &volumes, options, || {
    ...
    println!("Wrote {}: terminal={}, updates={}, compliance={:.9e} N*m, projected volume={:.9}",
        config.output.display(), terminal(report.termination), final_row.iteration,
        final_row.compliance, final_row.volume_fraction);
    Ok(())
}
