...
const badMode = JSON.parse(wasm.flyer_engine_init(1n, 1.294, 11.0, 3, 0, 18.3, 40n));
check("mode-invalid", badMode.refusal?.code === "mode-invalid", badMode);
const capOk = JSON.parse(wasm.flyer_engine_init(1n, 1.294, 20.0, 0, 0, 18.3, 2n));
check("headwind-at-cap-admits", !!capOk.ok, capOk);
const capPlus = JSON.parse(wasm.flyer_engine_init(1n, 1.294, 20.000000000000004, 0, 0, 18.3, 40n));
check("scenario-invalid", capPlus.refusal?.code === "scenario-invalid", capPlus);

// --- real Dec-17 init: identity fields --------------------------------------
const init = JSON.parse(wasm.flyer_engine_init(1903n, 1.294, 11.0, 0, 0, 18.3, 40n));
check(
  "init-identity",
...
const humanInit = JSON.parse(wasm.flyer_engine_init(7n, 1.294, 11.0, 2, 0, 18.3, 40n));
...
function transcript() {
  const parts = [wasm.flyer_engine_init(1903n, 1.294, 11.0, 0, 0, 18.3, 12n)];
