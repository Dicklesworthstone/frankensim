...
computed offsets; the leading checked-length validation rejects
`usize` overflow and bounds every offset used (max a: kk·8+6 with a
2-lane read => kc·8; max b: kk·4+2 with a 2-lane read => kc·4), and the accumulator
...
plane buffers after checked arithmetic rejects overflow. Each pair's
pointers are derived afresh from validated bases, advance only when a
subsequent `l` exists, and therefore never transiently leave the
allocation. Every access is 2 lanes. The packed kernel follows the
same rule with `mb`-strided pointers. The odd batch-lane tail routes
...
## Panic behavior
Checked bounds and conceptual tile-shape asserts fire BEFORE any unsafe
block. No unwinding between an intrinsic load and its paired store.
