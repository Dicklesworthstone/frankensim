...
> Status: PARTIAL — the GEMM section below is in force; factorizations,
> batched small-dense, and eigensolvers are still skeleton scope (their
> beads state their own contracts when they land).
...
## Error model
Slice-length mismatches panic with structured messages (programmer
errors). No other fallible paths in the GEMM layer.
...
- Factorizations, batched small-dense, eigensolvers: skeleton scope.
