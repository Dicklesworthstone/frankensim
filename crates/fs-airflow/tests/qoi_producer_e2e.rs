...
impl Emitter {
    fn new() -> Self {
        Self { body: String::new(), ordinal: 0, prev: None }
    }
    ...
    fn emit(&mut self, event: &str, fields: &[(&str, String)]) {
        ...
        let _ = write!(self.body, ",\"digest\":\"{digest}\"}}\n");
...
fn term_value_json(value: &TermValue) -> String {
    match value {
        ...
        TermValue::Negligible { justification } => {
            format!("{{\"state\":\"negligible\",\"why\":{}}}", json_str(justification))
        }
...
fn network() -> EnclosureNetwork {
    ...
    EnclosureNetwork::new(primary, LeakageElement::new(loss("leakage", 180_000.0, 0.25)))
}

fn linear_config() -> LinearConfig {
    LinearConfig { tolerance: 1.0e-10, max_iterations: 60_000, restart: 80 }
}

fn solve_config() -> SolveConfig {
    SolveConfig {
        initial: InitialGuess::Uniform(320.0),
        nonlinearity: Nonlinearity::FixedPoint { relaxation: 1.0, max_backtracks: 8 },
        stop: StopRule {
...
fn with_cx_gate<R>(gate: &CancelGate, seed: u64, kernel: u64, f: impl FnOnce(&Cx<'_>) -> R) -> R {
    ...
    pool.scope(|arena| {
        let cx = Cx::new(
            gate,
            arena,
            StreamKey { seed, kernel_id: kernel, tile: 0, iteration: 0 },
            Budget::INFINITE,
...
fn ambient_boundary(mesh: &ConductionMesh) -> ThermalBoundary {
    ThermalBoundaryBuilder::new(mesh)
        .region("ambient", |_| true, ThermalBc::dirichlet(300.0).expect("ambient"))
...
fn declarations(mesh: &ConductionMesh) -> (JunctionRegion, SurfaceRegion, FanPowerSpec) {
    ...
    let surface = SurfaceRegion::try_new(
        "case",
        (0..mesh.boundary().len()).rev().collect::<Vec<_>>(),
    )
    .expect("surface region");
    let power =
        FanPowerSpec::try_new(0.72, 0.04, source("producer-e2e-efficiency-v1")).expect("efficiency");
...
fn regime_point(flow_m3_s: f64) -> fs_regime::OperatingPoint {
    fs_regime::OperatingPoint {
        id: "solved".to_string(),
        groups: [("flow_m3_s".to_string(), flow_m3_s), ("speed_ratio".to_string(), 1.0)]
            .into_iter()
            .collect(),
    }
}

fn card_uses(qois: &fs_airflow::qoi::ThermalQoiSet, cards: &[&ModelCard]) -> Vec<ThermalQoiCardUse> {
    qois.budgets()
        .into_iter()
...
fn emit_inputs(emitter: &mut Emitter, temperatures: &[f64], operating: &fs_airflow::OperatingPoint) {
    ...
    emitter.emit(
        ...
        &[
            ...
            ("resistances", bits_list(&[40_000.0, 30_000.0, 12_000.0, 180_000.0])),
            ("fan_points", format!("[{}]", fan_points.join(","))),
            ("operating_flow_bits", bits(operating.flow.value.value())),
            ("operating_pressure_bits", bits(operating.pressure.value.value())),
...
fn produce(directory: &std::path::Path) -> Result<(), String> {
    ...
    emitter.emit(
        ...
        &[
            ...
            (
                "source_revision",
                json_str(
                    std::env::var("TQOI_SOURCE_REVISION").unwrap_or_else(|_| "unknown".into()).as_str(),
                ),
            ),
        ],
    );
    ...
    if requires("five_families_happy") {
        ...
        let set = extract_parts(&mesh, &solution, &operating, Some(&requirement_at(380.0, 1.25)))
            .map_err(|error| format!("happy extraction refused: {error}"))?;

        emit_qoi(&mut emitter, "junction_maximum", "kelvin", &set.junction_maximum.qoi);
        emit_qoi(&mut emitter, "pressure_drop", "pascal", &set.pressure_drop);
        emit_qoi(&mut emitter, "fan_power", "watt", &set.fan_power);
        emit_qoi(&mut emitter, "uniformity_mean", "kelvin", &set.uniformity.mean_temperature);
        emit_qoi(&mut emitter, "uniformity_spread", "kelvin", &set.uniformity.spread);
        emit_qoi(
            &mut emitter,
            "face_mean_std",
            "kelvin",
        ...
        emit_qoi(&mut emitter, "thermal_margin", "kelvin", &set.thermal_margin);
        ...
        let audited = set
            .clone()
            .audit_operating_envelope(std::slice::from_ref(&card), std::slice::from_ref(&point), &uses)
    ...
    if requires("narrowed_card_demotes_all") {
        ...
        let wide = set
            .clone()
            .audit_operating_envelope(std::slice::from_ref(&card), std::slice::from_ref(&point), &wide_uses)
        ...
        let all_demoted = tight.audit.receipts.iter().all(fs_regime::OutputClaimReceipt::demoted);
        ...
        emitter.emit(
            ...
            &[
                ...
                ("wide_coverage", json_str(&format!("{:?}", wide.audit.receipts[0].coverage))),
                ("narrow_coverage", json_str(&format!("{:?}", tight.audit.receipts[0].coverage))),
    ...
    if requires("tampered_conduction_refuses") {
        ...
        let contradiction =
            extract_parts(&mesh, &contradictory, &operating, Some(&requirement_at(380.0, 1.25)))
                .err()
                .ok_or("zero-receipt contradiction was accepted")?;
        if !matches!(contradiction, QoiError::InvalidInput { .. }) {
            return Err(format!("unexpected contradiction variant {contradiction:?}"));
        }
        ...
        let unconv_error =
            extract_parts(&mesh, &unconverged, &operating, Some(&requirement_at(380.0, 1.25)))
                .err()
        ...
        emitter.emit(
            ...
            &[
                ...
                ("refusal_second", refusal_json("qoi-invalid-input", &unconv_error)),
            ],
        );
    }
    ...
    if requires("deterministic_repeat_bitwise") {
        let journey_once = || -> Result<(Vec<f64>, [f64; 7], [String; 2]), String> {
            let operating = solve_network_operating_point();
            let (mesh, solution) = solve_real_conduction().map_err(|error| error.to_string())?;
            let set =
                extract_parts(&mesh, &solution, &operating, Some(&requirement_at(380.0, 1.25)))
            ...
            Ok((
                ...
                [
                    ...
                    set.uniformity.face_mean_standard_deviation.evidence.value.value(),
    ...
    emitter.emit("run_end", &[("cases", cases.len().to_string()), ("ok", "true".to_string())]);
...
fn verify(directory: &std::path::Path) -> Result<(), String> {
    ...
    for (index, line) in lines.iter().enumerate() {
        ...
        if recomputed != digest {
            return Err(format!("event {index}: digest mismatch (chain broken or tampered)"));
        }
        ...
        let prev_value = if let Some(rest) = prev_tail.strip_prefix('"') {
            ...
            prev_tail.split([',', '}']).next().unwrap_or_default().trim().to_string()
        };
        ...
        previous = Some(digest.to_string());
    }
    ...
    for required in cases {
        match *required {
            "five_families_happy" => {
                let records =
                    lines.iter().filter(|line| line.contains("\"event\":\"qoi_record\"")).count();
                if records < 7 {
                    return Err("happy case emitted fewer than seven qoi_record events".into());
                }
                if !lines.iter().any(|line| line.contains("\"event\":\"envelope_audit\"")) {
                    return Err("happy case emitted no envelope_audit event".into());
                }
            }
            "narrowed_card_demotes_all" => {
                if !lines.iter().any(|line| line.contains("\"event\":\"demotion\"")) {
                    return Err("narrowing case emitted no demotion event".into());
                }
            }
            "deterministic_repeat_bitwise" => {
                if !lines.iter().any(|line| line.contains("\"event\":\"determinism\"")) {
