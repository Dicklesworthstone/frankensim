...
fn no_adhesion_and_sink_monotone_bounded() {
    ...
    for (pen, rate) in [(0.02, 0.5), (0.04, 0.5), (0.05, 0.0), (0.03, -2.0), (0.01, -2.0)] {
        ...
        assert!(t.sink_m <= pen.max(prev_sink) + 1e-12, "sink bounded by penetration seen");
        prev_sink = t.sink_m;
    }
...
fn regularized_coulomb_saturates_and_is_linear_at_small_slip() {
    ...
    let frozen = ContactParams { sink_rate_per_s: 0.0, ..p };
    ...
    assert!((f_fast.abs() - frozen.mu * n).abs() < 0.001 * frozen.mu * n, "must saturate at muN");
    let (f_slow, _) = probe(0.005); // 0.1x v_reg: linear regime
    let expected = -frozen.mu * n * (0.005 / frozen.v_reg_mps);
    assert!((f_slow - expected).abs() < 0.01 * expected.abs(), "linear at small slip");
    // Odd symmetry.
    assert!((probe(0.3).0 + probe(-0.3).0).abs() < 1e-9);
    jlog("coulomb", &format!("\"saturated\":{},\"muN\":{}", f_fast.abs(), frozen.mu * n));
}
...
fn impulse_and_penetration_converge_under_dt_refinement() {
    ...
    for (name, a, b) in [
        ...
        ("penetration", r120.max_penetration_m, r480.max_penetration_m),
    ] {
        let rel = (a - b).abs() / b;
    ...
    assert!(e240 <= e120 + 1e-9, "peak-force refinement must not diverge");
    jlog(
        "convergence",
        &format!(
            "\"impulse\":[{},{}],\"pen\":[{},{}]",
            r120.normal_impulse_ns, r480.normal_impulse_ns, r120.max_penetration_m, r480.max_penetration_m
        ),
    );
}
...
fn impact_report_is_physical() {
    ...
    assert!(r.normal_impulse_ns > MASS * r.impact_speed_mps, "impulse under-absorbs the drop");
    assert!(r.peak_normal_n > MASS * G, "peak must exceed the static weight");
    ...
    assert!(r.sliding_m < 25.0, "slide {} m implausible for sand at mu 0.5", r.sliding_m);
    jlog(
        "report",
        &format!(
...
fn refusals_at_cap_and_cap_plus_one() {
    ...
    assert_eq!(
        contact_tick(&p, &mut st, above, 0.0, 0.0, dt).unwrap_err().code,
        "penetration-outside-domain"
    );
    let bad = ContactParams { stiffness_n_m: 0.0, ..p };
    assert_eq!(
        contact_tick(&bad, &mut st, 0.01, 0.0, 0.0, dt).unwrap_err().code,
        "contact-params-invalid"
    );
    assert_eq!(
        contact_tick(&p, &mut st, f64::NAN, 0.0, 0.0, dt).unwrap_err().code,
        "non-finite-input"
    );
    jlog("refusals", "\"gates\":\"penetration cap/cap+1, params, NaN\"");
}
