...
The systematic build-configuration evidence is the committed
`trust-root-determinism-matrix.jsonl` (bead f85xj.3.4, regenerated by
`scripts/ci/trust_root_determinism_matrix.sh`): the full fs-math + fs-ivl
batteries green across {aarch64-apple-darwin, x86_64-unknown-linux-gnu} ×
{debug, release} on the pinned nightly, with next-nightly and alternate
codegen flags recorded as explicit NOT-RUN cells rather than omitted. Run
the matrix before and after any toolchain bump.
