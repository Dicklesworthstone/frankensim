...
pub(super) fn run(args: &[String]) -> Result<u8, Box<dyn Error>> {
    if args.len() < 2 || args.len() > 8 {
        return Err("usage: fs-marquee-elasticity-stress --projected OUTPUT_DIR MAX_SAMPLED_VON_MISES [LEVEL=3] [ITERATIONS=30] [VOLFRAC=0.6] [MAX_CANDIDATES=8] [ABS_STRESS_TOL=0] [WALL_SECONDS=300]".into());
    }
    ...
    let mut optimizer = ProjectedStressOptimizer::new(area_optimizer, limit)?;
    ...
    let status = loop {
        ...
        match update.progress {
            ProjectedProgress::Accepted(step) => {
                ...
                // Retain every accepted field and its evidence before starting
                // more physics. A failed write never prints a success summary.
                trajectory.flush()?;
    ...
    let summary = format!(concat!(
        "{{\"schema\":\"fs-marquee-projected-stress-v1\",",
        ...
        max_candidates, wall_seconds, volfrac, projection.tolerance,
    ...
    println!("{summary}");
    Ok(match status { "iteration_limit" => 0, "wall_budget" => 6, _ => 11 })
}
