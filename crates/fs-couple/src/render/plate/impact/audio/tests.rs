...
#[test]
fn prepared_contact_image_reaches_the_same_radiation_decimator_and_pcm_stream() {
    use super::super::{BodyPotential, linear::{LinearImpactSystem, LinearImpactConfig}};
    ...
    let make = |contact_enabled: bool| {
        ...
        let config = LinearImpactConfig {
            sample_rate_hz: RATE, max_steps: 1028, maximum_generalized_force: 1000.0,
            component: ModalAcousticTimeBudget::audible_reference(),
            coupling: ModalCouplingConfig { max_modes: 16, max_connections: 4, max_setup_terms: 10000,
                nyquist_guard_fraction: 0.9, maximum_total_energy_j: 10.0,
                maximum_abs_pressure_pa: 1e6, maximum_abs_connection_force_n: 1e4,
                solve_relative_tolerance: 1e-10, energy_absolute_tolerance_j: 1e-10,
                energy_relative_tolerance: 1e-7 },
            contact: ModalContactConfig { max_iterations: 100, maximum_force_n: 1000.0,
                maximum_penetration_m: 0.01, force_absolute_tolerance_n: 1e-10,
                force_relative_tolerance: 1e-9 },
            multiple: MultiContactConfig { max_contacts: 16, max_sweeps: 100, max_setup_terms: 10000 },
        };
        let mechanics = LinearImpactSystem::new(vec![striker, receiver],
            if contact_enabled {vec![contact]} else {vec![]}, vec![], config,
    ...
    assert!(baseline.source().last_mechanical_frame().unwrap().maximum_contact_residual_n < 1e-6);
