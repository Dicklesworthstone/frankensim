...
pub fn option(args: &mut Vec<String>) -> Result<Option<Program>, Error> {
    ...
    let mut i = 0;
    while i < args.len() {
        if args[i] != "--stick-force-file" {
            i += 1;
            continue;
        }
        if selected.is_some() {
            return Err("--stick-force-file may be supplied only once".into());
        }
        let path = args.get(i + 1).ok_or("--stick-force-file needs a CSV path")?;
        if path.starts_with("--") {
            return Err("--stick-force-file needs a CSV path, not another option".into());
        }
...
/// tick. Only `accept` advances the playing clock after the mechanical commit.
...
impl StickDrive {
    pub fn new(program: Program, dt_s: f64, steps: u64, tip_weight: f64, modes: usize) -> Result<Self, Error> {
        program.admit(dt_s, steps, tip_weight)?;
        if modes == 0 {
            return Err("stick drive needs the existing mechanical force basis".into());
        }
        Ok(Self { program, dt_s, steps, accepted: 0, tip_weight, force: vec![0.0; modes] })
    }
    ...
    pub fn forces(&mut self, external: &[f64]) -> Result<&[f64], ImpactError> {
        ...
        // Both reference constructors place the geometrically integrated,
        // fixed-axis stick coordinate first. F/sqrt(m) is work-conjugate to
        // q=sqrt(m)*x. No force is injected directly into a head or air mode.
        self.force[0] += self.tip_weight * self.program.average(begin, end);
        if !self.force[0].is_finite() {
