...
fn fg_001_no_card_refuses_and_a_refusal_boundary_card_cannot_drive() {
    let geom = geometry(0.004, 0.0, 0.0);
...
fn fg_002_energy_ledger_closes_to_roundoff_every_block() {
    ...
    let mut v = voice(Some(&card), geometry(0.004, 0.0, 0.0), 2, 0).expect("voice");
...
fn fg_003_overblowing_ladder_walks_fundamental_to_octave() {
    ...
    let mut map = Vec::new();
    println!("REGIME MAP (blow_pa, U, Re, inside_card, transit_ms, lock_hz, lock/f1, p_rms):");
    for &p in &pressures {
        let (lock, rms) = settle(&mut v, p, 60);
        let d = *v.diagnostics().last().expect("diag");
        println!(
            "{p:.0}\t{:.2}\t{:.0}\t{}\t{:.3}\t{lock:.1}\t{:.3}\t{rms:.3e}",
            d.jet_speed_m_s,
            d.reynolds,
    ...
    assert!(
        (1.7..2.4).contains(&ratio_high),
        "highest pressure locks near the octave: {ratio_high}"
    );
}
...
fn fg_004_voicing_gestures_move_onset_and_duty_in_the_expected_direction() {
    ...
    let long = onset(0.006);
    println!(
        "{{\"fixture\":\"fg-004\",\"onset_pa_cut_up_3mm\":{short:?},\"onset_pa_cut_up_6mm\":{long:?}}}"
    );
    let (Some(short), Some(long)) = (short, long) else {
        panic!("both cut-ups must reach onset in the sweep");
    };
    assert!(
        long > short,
        "a longer cut-up needs more pressure to start: {short} vs {long}"
    );

    let duty = |offset: f64| -> f64 {
        let mut v = voice(Some(&card), geometry(0.004, offset, 0.0), 4, 1).expect("voice");
        let _ = settle(&mut v, 300.0, 40);
...
fn fg_006_replay_is_bitwise_and_keyed_by_voice_identity() {
    ...
    let render = |index: u32| -> Vec<f64> {
        let mut v = voice(Some(&card), geometry(0.004, 0.0, 0.0), 9, index).expect("voice");
...
fn fg_007_claims_outside_the_card_validity_region_are_refused() {
    let card = mint_tonal_interim_card().expect("card");
    let mut v = voice(Some(&card), geometry(0.004, 0.0, 0.0), 11, 0).expect("voice");
    ...
    // Inside the band the gate admits: a jet slow enough to sit in the
    // card's Reynolds band (about 3 Pa for this flue).
    let mut slow = voice(Some(&card), geometry(0.004, 0.0, 0.0), 11, 1).expect("voice");
    let _ = settle(&mut slow, 3.0, 1);
