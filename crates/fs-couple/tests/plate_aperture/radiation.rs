...
fn failed_physics_and_cancelled_audio_preserve_radiation_material_and_receiver_history() {
    ...
    assert_eq!(render_pressure_pcm16(&mut actual,&mut pcm,&gate,&mut scratch,SAMPLES as usize-271).unwrap(),ScheduledWavProgress::Cancelled{samples:0});
    assert_eq!(scratch,[-123.0;37]);assert_eq!(actual.system().aperture().state(),before);
    assert_eq!(actual.pending_controls(),pending);
    render_pressure_pcm16(&mut actual,&mut pcm,&CancelGate::new(),&mut scratch,SAMPLES as usize-271).unwrap();
