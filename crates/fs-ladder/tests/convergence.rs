...
fn test_convergence_insufficient_rungs_fails_closed() {
    // 1. One rung only
    let plan1 = ConvergencePlan::new("junction_maximum", 2.0).with_rung(MeshRung::new(
        0,
        "mesh_coarse",
        0.04,
        "m",
    ...
    let plan2 = plan1.with_rung(MeshRung::new(
        1,
        "mesh_medium",
        0.02,
        "m",
...
fn test_convergence_monotone_asymptotic_order_2() {
    ...
    assert!(
        (observed - 2.0).abs() < 1e-4,
        "observed order should be 2.0, got {observed}"
    );

    let extrap = res.richardson_extrapolated_qoi.expect("extrapolated value");
    assert!(
        (extrap - 340.0).abs() < 1e-4,
        "extrapolated value should be 340.0, got {extrap}"
    );
...
fn test_convergence_oscillatory_refuses_extrapolation() {
    ...
    assert!(
        res.richardson_extrapolated_qoi.is_none(),
        "extrapolation must be refused on oscillatory ladder"
    );
