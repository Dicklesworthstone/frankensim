...
fn policy(
    memory_bytes: u64,
    solid: LinearGoalAnalysisConfig,
    linear: LinearConfig,
...
fn cancelled() -> SolveRefusal {
    conduction_error(
        "cli-solve-cancelled",
        "coupled maximum analysis was cancelled",
        "rerun the solve; no partial coupled evidence was published",
    )
}

// The QoI uncertainty reason has a bounded size. Keep the complete diagnostic
// in solver_control and give the budget a short, stable explanation.
fn gap(
    stage: &str,
    diagnostic: String,
    requested: Option<f64>,
    response_iterations: Option<usize>,
) -> Result<MaximumEvidence, SolveRefusal> {
    Ok(MaximumEvidence {
        term: Some(PropagatedTerm::Unmeasured {
            reason: format!(
                "full solid/air maximum {stage} unavailable; see coupled solver_control reason; no frozen-solid or tolerance fallback"
            ),
        }),
        control_json: Some(format!(
            "{{\"schema\":{},\"status\":\"coupled-analysis-unavailable\",\"mode\":\"assessment-only\",\"goal_met\":false,\"correction_supported\":false,\"candidate_accepted\":false,\"requested_tolerance_k\":{},\"primal_iterations\":0,\"response_iterations\":{},\"reason\":{},\"scope\":{}}}",
            json_string(SCHEMA),
            optional_number(requested)?,
            response_iterations.map_or_else(|| "null".to_string(), |count| count.to_string()),
...
pub(super) fn maximum_evidence(
    ...
    let analyzer = match fs_airflow::conjugate::goal::maximum::prepare_linear_maximum(
        ...
        Err(error) => {
            ...
            return gap(
                "preparation",
                format!(
                    "full solid/air maximum preparation refused: {error}; preparation work is unavailable"
                ),
                requested,
                None,
            );
    ...
    let analysis = match analyzer.analyze_maximum(cx, temperature, vertices) {
        ...
        Err(error) => {
            ...
            return gap(
                "evaluation",
                format!("full solid/air maximum evaluation refused: {error}"),
                requested,
                Some(analyzer.response_iterations()),
            );
        }
    };
    let coupled = analysis.coupled();
    let response_defect = coupled
        .response_residual_infinity_upper()
        .iter()
        .copied()
        .reduce(f64::max);
    ...
    let status = if bound.is_none() {
        "coupled-bound-unavailable"
    } else if goal_met {
        "coupled-goal-tolerance"
    } else {
    ...
    let detail = format!(
        ...
        analysis.nominal_k(),
        analysis.interval_k(),
        coupled.residual_infinity_upper(),
        coupled.solid_inverse_infinity_upper(),
        coupled.gain_infinity_upper(),
        coupled.coupled_inverse_infinity_upper(),
        coupled.inverse_method(),
        coupled.schur_inverse_infinity_upper(),
        coupled.status(),
        analysis.response_iterations(),
        config.max_response_iterations,
        coupled.response_residual_infinity_upper().len(),
        requested,
        goal_met,
    );
    let term = match bound {
        ...
        None => PropagatedTerm::Unmeasured {
            reason: format!(
                "no finite coupled maximum bound: {:?}; a failed sufficient check is not a singularity claim; see coupled solver_control for residual, inverse and response diagnostics; no frozen-solid or tolerance fallback",
                coupled.status()
            ),
        },
    };
    let inverse_method = coupled
        .inverse_method()
    ...
    let control_json = format!(
        ...
        analysis.response_iterations(),
        config.max_response_iterations,
        solid_config.max_stability_iterations,
        paths.len(),
        analyzer.ports().len(),
        ...
        optional_number(response_defect)?,
        inverse_method,
