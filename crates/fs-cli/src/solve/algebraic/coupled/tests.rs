...
impl Fixture {
    fn new() -> Self {
        ...
        let paths = vec![
            AirPath::new(
                ...
                1.0,
                24.0,
                vec![AirSegment::new("air", area, 2.0).unwrap()],
            )
            .unwrap(),
        ];
        Self {
    ...
    fn assess(
        ...
        self.assess_selected(
            cx,
            field,
            memory,
            requested,
            &(0..field.len()).collect::<Vec<_>>(),
        )
    }
    fn assess_selected(
        &self,
...
fn coupled_product_term_sees_the_error_hidden_by_frozen_robin_and_reaches_qoi() {
    ...
    with_cx(|_, cx| {
        ...
        let terms = super::super::super::propagation_term_receipts(
            None,
            None,
            evidence.term.as_ref(),
            fs_blake3::hash_bytes(b"coupled-conduction-receipt"),
        )
        .unwrap();
        assert_eq!(
            terms
                .iter()
                .filter(
                    |term| term.kind() == fs_evidence::uncertainty::EngineeringUncertaintyKind::SolverAlgebraic
                )
                .count(),
            1
        );
        ...
        let near = fixture
            .assess(cx, &vec![330.0; field.len()], 64 * 1024 * 1024, 1e-6)
            .unwrap();
...
fn coupled_product_gaps_are_not_replaced_by_frozen_or_tolerance_estimates() {
    ...
    with_cx(|_, cx| {
        ...
        assert!(matches!(
            refused.term,
            Some(PropagatedTerm::Unmeasured { .. })
        ));
        let json = JsonValue::parse(refused.control_json.as_ref().unwrap()).unwrap();
        assert_eq!(
            json.str_field("status"),
            Some("coupled-analysis-unavailable")
        );
        assert_eq!(json.get("goal_met"), Some(&JsonValue::Bool(false)));
        assert_eq!(json.get("response_iterations"), Some(&JsonValue::Null));
        assert_eq!(json.f64_field("requested_tolerance_k"), Some(1.0));
        assert_eq!(
            json.get("correction_supported"),
            Some(&JsonValue::Bool(false))
        );
        // An invalid region is detected after response preparation. Its spent
        // work must survive the refusal, and the gap must reach the real QoI.
        let refused = fixture
            .assess_selected(cx, &field, 64 * 1024 * 1024, 1.0, &[field.len()])
            .unwrap();
        let json = JsonValue::parse(refused.control_json.as_ref().unwrap()).unwrap();
        assert!(json.f64_field("response_iterations").unwrap() > 0.0);
        let terms = super::super::super::propagation_term_receipts(
            None,
            None,
            refused.term.as_ref(),
            fs_blake3::hash_bytes(b"coupled-refusal"),
        )
        .unwrap();
        assert_eq!(
            terms
    ...
    fixture.paths = vec![
        AirPath::new(
            ...
            1.0,
            24.0,
            vec![AirSegment::new("air", 7.0, 2.0).unwrap()],
        )
        .unwrap(),
    ];
    with_cx(|_, cx| {
        let refused = fixture
            .assess(
                cx,
                &vec![300.0; fixture.mesh.vertex_count()],
                64 * 1024 * 1024,
                1.0,
            )
            .unwrap();
    ...
    with_cx(|gate, cx| {
        ...
        for request in [0.0, -1.0, f64::NAN, f64::INFINITY] {
            let evidence = fixture
                .assess(cx, &field, 64 * 1024 * 1024, request)
                .unwrap();
