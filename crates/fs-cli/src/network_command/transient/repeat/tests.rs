...
fn repeated_pulses_accumulate_heat_and_account_for_every_cycle() {
    ...
    close(repeated.f64_field("sampled_peak_objective_k").unwrap(),309.1035042282367,1e-4);
    close(repeated.f64_field("sampled_peak_time_s").unwrap(),1380.0,0.0);
    close(field(&doc).into_iter().fold(f64::NEG_INFINITY,f64::max),305.5585428961678,1e-4);
    let cycles=repeated.get("cycles").unwrap().as_array().unwrap();
    assert!(cycles[0].f64_field("sampled_peak_objective_k").unwrap()<306.0);
    assert!(cycles[2].f64_field("sampled_peak_objective_k").unwrap()>306.0);
...
fn power_sizing_keeps_repetitions_and_does_not_reset_each_cycle() {
    ...
    close(multiplier,5.5/(306.54498944826173-300.0),0.001);
    assert!(design.f64_field("sampled_peak_objective_k").unwrap()<=305.5);
    assert!(design.path(&["failed_upper","sampled_peak_objective_k"]).unwrap().as_f64().unwrap()>305.5);
