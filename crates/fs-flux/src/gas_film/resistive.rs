...
fn fail<T>(s: &'static str) -> Result<T, FilmError> { Err(FilmError(s)) }
...
impl ResistiveFilm {
    ...
    pub fn new(cells: Vec<FilmCell>, channels: Vec<FilmChannel>, ports: usize,
        viscosity_pa_s: f64, limits: FilmLimits) -> Result<Self, FilmError>
    {
        if ports == 0 || ports > MAX_PORTS || cells.is_empty() || cells.len() > MAX_CELLS
            || channels.is_empty() || channels.len() > MAX_CHANNELS
            || !viscosity_pa_s.is_finite() || viscosity_pa_s <= 0.0
            || !limits.minimum_cell_gap_m.is_finite() || limits.minimum_cell_gap_m <= 0.0
            || !limits.maximum_gap_m.is_finite() || limits.maximum_gap_m <= limits.minimum_cell_gap_m
            || !limits.maximum_pressure_pa.is_finite() || limits.maximum_pressure_pa <= 0.0
        { return fail("invalid film dimensions, viscosity, or validity limits"); }
        let valid_gap = |g: &GapPort| g.reference_m.is_finite()
            && g.closure.len() == ports && g.closure.iter().all(|x| x.is_finite());
        for cell in &cells {
        ...
        for edge in &channels {
            if edge.from >= cells.len() || edge.to.is_some_and(|j| j >= cells.len() || j == edge.from)
                || !valid_gap(&edge.gap) || !edge.width_m.is_finite() || edge.width_m <= 0.0
                || !edge.length_m.is_finite() || edge.length_m <= 0.0
            { return fail("invalid film channel geometry or connectivity"); }
            let factor = (edge.width_m / edge.length_m) / (12.0 * viscosity_pa_s);
            if !factor.is_finite() || factor <= 0.0 { return fail("film conductance scale overflow"); }
            factors.push(factor);
        }
        ...
        Ok(Self { cells, channels, factors, ports, limits })
    }

    /// Number of work-conjugate mechanical coordinates.
    pub fn port_count(&self) -> usize { self.ports }
    ...
    fn solve(&self, q: &[f64], v: &[f64]) -> Result<Solution, FilmError> {
        ...
        for (k, (edge, factor)) in self.channels.iter().zip(&self.factors).enumerate() {
            ...
            if !g.is_finite() { return fail("nonfinite film conductance"); }
            s.conductance[k] = g;
            let i = edge.from;
            s.lower[i*n+i] += g;
        ...
        if s.pressure[..n].iter().any(|p| p.abs() > self.limits.maximum_pressure_pa) {
    ...
    pub fn evaluate_into(&self, q: &[f64], velocity: &[f64], force: &mut [f64],
        pressure: &mut [f64]) -> Result<FilmReport, FilmError>
    {
        ...
        if candidate[..self.ports].iter().any(|x| !x.is_finite()) || !power.is_finite() || !work.is_finite()
        ...
        Ok(FilmReport { minimum_gap_m: s.minimum_gap,
    ...
    pub fn tangent_into(&self, q: &[f64], velocity: &[f64], dq: &[f64], dv: &[f64],
        out: &mut [f64]) -> Result<(), FilmError>
    {
        if dq.len() != self.ports || dv.len() != self.ports || out.len() != self.ports
    ...
    fn project(&self, p: &[f64; MAX_CELLS], f: &mut [f64; MAX_PORTS]) {
        for (i, cell) in self.cells.iter().enumerate() {
            ...
            for (j, weight) in cell.gap.closure.iter().enumerate() { f[j] += weight * force; }
        }
    }
}
...
impl Solution {
    fn new(n: usize, minimum_gap: f64) -> Self {
        Self { lower: [0.0; MAX_CELLS*MAX_CELLS], scale: [0.0; MAX_CELLS],
            pressure: [0.0; MAX_CELLS], conductance: [0.0; MAX_CHANNELS], n, minimum_gap }
    }
    fn factor(&mut self) -> Result<(), FilmError> {
        let n = self.n;
        for i in 0..n {
            let d = self.lower[i*n+i];
            if !d.is_finite() || d <= 0.0 { return fail("undrained or nonfinite film pressure cell"); }
            self.scale[i] = d.sqrt();
        }
        ...
        for i in 0..n { for j in 0..=i {
            ...
            for k in 0..j { sum -= self.lower[i*n+k] * self.lower[j*n+k]; }
            if i == j {
...
fn solve_factored(l: &[f64; MAX_CELLS*MAX_CELLS], scale: &[f64; MAX_CELLS],
    n: usize, rhs: &mut [f64; MAX_CELLS]) -> Result<(), FilmError>
{
    for i in 0..n {
        let mut x = rhs[i] / scale[i];
        for j in 0..i { x -= l[i*n+j] * rhs[j]; }
        rhs[i] = x / l[i*n+i];
    }
    for i in (0..n).rev() {
        let mut x = rhs[i];
        for j in i+1..n { x -= l[j*n+i] * rhs[j]; }
        rhs[i] = x / l[i*n+i];
    }
    for i in 0..n { rhs[i] /= scale[i]; }
    if rhs[..n].iter().any(|x| !x.is_finite()) { return fail("nonfinite film pressure solution"); }
    Ok(())
}
