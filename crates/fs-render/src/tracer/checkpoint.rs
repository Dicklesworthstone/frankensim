...
fn encode_uniform_state<S: BodySink>(
    sink: &mut S,
    settings: &Settings,
    layout: RenderTileLayout,
    ...
    for tile in 0..layout.tile_count {
        ...
        encode_tile_header(sink, tile, bounds, state.next_row[tile as usize])?;
...
fn encode_adaptive_state<S: BodySink>(
    sink: &mut S,
    settings: &Settings,
    layout: RenderTileLayout,
    state: &PendingAdaptiveRenderState,
) -> Result<(), S::Error> {
    for tile in 0..layout.tile_count {
        let bounds = layout
            .bounds(tile)
        ...
        for y in bounds.y..bounds.y + bounds.height {
            for x in bounds.x..bounds.x + bounds.width {
                ...
                put_u32(sink, state.film.sample_counts[pixel])?;
                put_u8(sink, state.film.decisions[pixel] as u8)?;
            }
        }
    }
    Ok(())
...
impl PendingRender<'_> {
    ...
    pub fn write_checkpoint<E>(
        ...
        let progress = pending_progress(self.layout, &state.next_row, self.attempts);
        ...
        encode_uniform_state(&mut sink, &self.settings, self.layout, &state)?;
...
impl PendingAdaptiveRender<'_> {
    ...
    pub fn write_checkpoint<E>(
        ...
        encode_adaptive_state(&mut sink, &self.settings, self.layout, &state)?;
        let (digest, body_len) = sink.finish_body()?;
        let byte_len = finish_stream(cx, &mut emit, digest, body_len)?;
        debug_assert_eq!(byte_len, required);
        Ok(RenderCheckpointReceipt {
            ...
            for local_y in 0..bounds.height {
                for x in bounds.x..bounds.x + bounds.width {
                    ...
                    if vectors.iter().flatten().any(|value| !value.is_finite())
                        || vectors[2].iter().any(|value| *value < 0.0)
                    {
                        return Err(RenderCheckpointError::InvalidPixelState);
                    }
                    if committed {
