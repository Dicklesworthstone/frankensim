...
fn linear_model_reduces_exactly_to_the_existing_scalar_updater() {
    ...
    with_cx(&gate, |cx| {
        ...
        assert!(bits_eq(result.linearization().prediction(), 5.0));
        assert!(bits_eq(result.linearization().innovation(), 0.5));
...
fn probes_retain_actual_states_and_receipts_bind_model_identity() {
    ...
    with_cx(&gate, |cx| {
        ...
        assert!(bits_eq(first.prediction(), 1.0));
        assert_eq!(first.jacobian(), &[4.0, 3.0]);
        assert_eq!(first.probes().len(), 2);
        assert_eq!(first.probes()[0].component(), 0);
        assert!(bits_eq(first.probes()[0].nominal_step(), 0.5));
        assert!(bits_eq(first.probes()[0].plus_state(), 2.5));
        assert!(bits_eq(first.probes()[0].minus_state(), 1.5));
        assert!(bits_eq(first.probes()[0].plus_prediction(), 3.25));
        assert!(bits_eq(first.probes()[0].minus_prediction(), -0.75));
        assert!(bits_eq(first.probes()[0].denominator(), 1.0));
        assert_eq!(first.probes()[1].component(), 1);
        assert!(bits_eq(first.probes()[1].nominal_step(), 0.25));
        assert!(bits_eq(first.probes()[1].plus_state(), -0.75));
        assert!(bits_eq(first.probes()[1].minus_state(), -1.25));
        assert!(bits_eq(first.probes()[1].plus_prediction(), 1.75));
        assert!(bits_eq(first.probes()[1].minus_prediction(), 0.25));
        assert!(
            first
                .receipt_id()
...
fn large_innovation_is_demoted_without_observation_or_posterior() {
    ...
    with_cx(&gate, |cx| {
        ...
        assert!(bits_eq(
            result.linearization().standardized_innovation(),
            100.0
        ));
