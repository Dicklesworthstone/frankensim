...
/// exact volume from its tracked bytes. Its rotated twin (the generator's
/// `--rotate`/`--shift`) does NOT yet: with four fins the rotated point set
/// carries coplanar clusters whose zero-volume tets chain, every fan
/// re-triangulation mints another flat, and the winding audit refuses —
/// the next B4 increment (bead q61wp.46), measured 2026-09-02.
#[test]
