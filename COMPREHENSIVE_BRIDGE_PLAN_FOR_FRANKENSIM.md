...
**B8a. Export verbs take 60–130 s on a 7-stage ledger (q61wp.62, CLOSED e26b5428).** *State:* measured in both shell lanes (65 s / 133 s at moderate load; 107 s / 123 s at load 860). *Found:* exports re-solved the physics — the resume validator replayed flow-network, conduction and QoI to compare rebuilt receipts bitwise with the retained ones (3.4 s of a 4.2 s validation on the heatsink). *Landed:* longest-prefix-first validation, one-statement artifact lookups, three-statement op reads (d320fb5b); then `ResumeProof::{Replay, SealedEvidence}` — `solve --resume` still replays, `report`/`package` prove by sealed evidence (receipt re-hash against the sealed driver state, conduction outputs from the sealed op with kinds checked) and disclose `"verification":"sealed-evidence"`. *Measured:* report 0.49 s, package 0.47 s on a ledger whose solve took 6 s; candidate validation 0.24 s (was 3.97 s). *Residual:* the controlled op read still pays a `sqlite_master` identity attestation per call (5 statements, 11 ms per op); cache it behind `PRAGMA schema_version` only if a larger ledger makes it visible.
...
- **Landings, execution pass 3 (2026-09-02, 02:45–08:30):** B2a (flat-tet repair by edge removal in `fs-mesh`; comb 9 flat tets → 0, min dihedral 6.4°; conduction receipt v5 with `mesh.quality` and recovery evidence incl. `flat_tets`), B2b (quality floor: a surviving flat tet or min dihedral < 1° refuses `cli-solve-conduction-mesh-quality`), B8a closed (exports 60–130 s → 0.5 s; cause was physics replay on export; `ResumeProof` split, disclosed as `verification`), A13 partial (`cli-solve-run-incompatible` names the count and reason of inadmissible retained operations), truth repair: fs-cli CONTRACT's verb paragraph still claimed `report`/`package` refuse `cli-stage-unavailable` — rewritten. Tropical critical-path artifact repaired (invalid JSON; deferred beads counted as live). **B2 ladder slice (306a03ef + the fs-cli commit that follows):** `refine_uniform` in fs-mesh; `solver.fidelity = "ladder"` runs three uniform rungs, receipt v6 + QoI receipt v2 + report convergence section; the FIRST measured budget term (discretization, 1.1 mK data-range on the heatsink); linear gate follows `tolerance-rel`; fs-report's convergence JSON block (dead code writing Rust debug text) fixed; freshness lane 22 → 33 checks; three more stale no-claim bullets in fs-cli CONTRACT rewritten. **B4 kernel slice (fs-mesh):** the body corpus (`tests/body_corpus.rs`: rotated two-fin comb, plate with a through-hole) found and fixed a Delaunay kernel defect — the coplanar ghost rule's apex `a + n` rounded onto the plane on near-collinear hull slivers, growth repair then swallowed a hull vertex on every rotated body (real CAD is never axis-aligned); plus the audit now checks vertex presence, `recover()` audits first, and the flat-tet repair handles wall-touching and boundary-embedded slivers. Rotated comb: 247 tets, exact volume, no flat tets; the tracked heatsink shell meshes from its bytes; the four-fin shell rotated 35°/21° still refuses (coplanar sliver chains — the next B4 increment; a nine-decimal copy is worse, so STL precision is part of meshability). B2 WORK 1 (constrained refinement) landed as infrastructure with interior-only insertion and measured evidence; wall splitting is opt-in and unclaimed (needs a CDT-consistent facet split).
- **Landings, execution pass 3 continued (2026-09-02, 13:00–20:00) — B4 recovery slice (fs-mesh):** the rotated four-fin shell's "sliver fans" were a symptom. A new per-region closed-tile-surface check (refusal `recovered facet tiles do not close the region surface`) found ONE hole: two facets meeting at a crease tiled their shared segment with different vertex chains (one across the original edge, one through the midpoint minted on it — both passed the per-facet test), the seed flood walked through it, and the audit tripped on an exterior zero-volume tet far away. The edge survived its own midpoint because nearly coplanar HULL layers carry flat tets whose circumspheres are so large that an f64 midpoint a rounding hair off the plane lies outside them (14 such midpoints on that shell). Coplanar tilings then failed because every co-circular quad carries the kernel's zero-volume tet with BOTH triangulations as faces, so "all in-plane faces" is a double cover (one strip reached 4614 sub-triangles). Fixes: tiling free edges must equal the chord chains of every mesh vertex on each facet edge; faces skipping a chain vertex are never tiles; a present own sub-triangle that skips one counts as missing (balanced bisection adopts it — fanning over the chain was a measured dead end: 3393 of 3703 Steiner points on the comb that needs 136); the coplanar tiling is the sheet on one side of the plane (boundary of that side's tets; both sides tried), the historical answer kept byte-identical. Corpus: `heatsink-rotated.stl` tracked and pinned (378 tets, 154 vertices, 98 Steiner, exact volume, two interior flats disclosed); the same shell at f32 precision (what fs-io's STL reader welds) meshes too (727 tets, exact volume); axis-aligned bodies unchanged; fs-mesh and fs-cli suites green. CONTRACT items 20–21. The CLI refusal that remained was resolved in the next entry.
- **Landings, execution pass 3 continued (2026-09-02 23:00 – 2026-09-03 03:00) — B4 closed through the product path:** the crate replayed the CLI's exact point set with an identical recovery trace, so the divergence was after recovery; the probe with reversed triangles reproduced the refusal exactly. Root cause (c17a7906): `fs_rep_mesh::repair::unify_orientation` decided a shell's global orientation from the winding number at the VERTEX centroid, which for a finned body lies in the air, where the winding is a 1e-16 rounding residual of arbitrary sign — the rotated shell was inverted at STL import, and fs-io reports the enclosed volume as |V| so nothing downstream could tell. The vote is now the signed enclosed volume (conformance `rmesh-006` pins both starting orientations of a comb; the supplier-corpus golden changed by one receipt's text only, all eleven shells' decisions unchanged), and fs-cli's `region_specs` orients each region from its own surface's signed volume (4474e5d5). The CLI then stopped at the 1° mesh-quality floor: f32 import precision leaves near-flat slivers (volume 1e-6 of the largest, dihedral 1e-6°) the flat repair never saw. Landed: sliver repair by dihedral with volume-inclusive conservation (4720b6fe, 28 → 2 slivers), needle removal (the stale edge whose chord carries another vertex of the tet) and up to three rounds of Steiner perturbation with rebuild, one vertex per survivor along its own constraint, kept only on strict improvement (ea86edbd, 2 → 0 at f64 and f32). RESULT: `heatsink-fan-rotated.fsim` imports, solves all seven stages and reports; axis-aligned T_max 301.99578 K (713 tets) vs rotated 301.99562 K (704 tets), a 0.16 mK difference on an 8.85 K rise, inside the 1.1 mK discretization bound — the freshness lane's step 9 requires agreement within 1 mK. Also: a HEAD compile break in `fs-mbd` (another agent's module importing the crate's own `Vec3` where `JointModel::revolute` takes `fs_ga::Vec3`) blocked every CLI build and was fixed (63516eda); the NVMe filled to 38 MB free mid-pass and was cleared by the owner (measured command list in memory); the host ran at load 500–1000 for hours, starving builds.
- **Landings, execution pass 3 continued (2026-09-03, 15:30–17:00) — B4 complexity gate and C3 enclosure.** Carried to HEAD inside another agent's tree commit `4836e81d`, whose message is not this work's; the measurements are here. Probing the gate with tessellated cylinders showed the cost was never recovery — at 1280 facets all facets recover with ZERO Steiner points, because the Delaunay already contains the surface — but the flat/sliver repair. It swept a fresh face map per round inside a fixed `0..32` loop, so it stopped after 32 removals however many candidates existed (767 found, 17 removed, 5 s). It now sweeps every candidate against one face map and rebuilds only after a commit; zero-volume tets go first and unbudgeted (sharing one queue with slivers left 53 fatal tets where priority leaves 1) while dihedral slivers share a flip-attempt cap; the Steiner perturbation moves a bounded number of vertices per round and is skipped once more than that many tets survive. Whole pipeline 12 s → 1.3 s on that body. Recorded as a NO-CLAIM (fs-mesh CONTRACT item 24): a co-circular tessellation is not made usable by repair — its census (1373 tets under 5°, radius-edge 1642) is refused by the conduction quality floor and the body needs refinement, not repair. New corpus body for **C3**: a vented enclosure, a box shell whose interior void reaches the outside through a slot so no part of it is a sealed cavity and every wall is a thin feature — 188 facets recovered with a single Steiner point, 1137 tets, exact volume, smallest dihedral 2.0°. Its first draft was non-manifold, which is the reusable lesson for generators: a face carrying a vent edge must subdivide its neighbours' shared edges too, so every face of both boxes is emitted on one shared breakpoint grid. Verified: fs-mesh green in debug (corpus 6/6 in 13.6 s, conformance 26/26) and release, rotated heatsink unchanged at f64 and f32.
- **Landings, execution pass 3 continued (2026-09-03, 17:00–18:10) — B4 item (b), first slice.** Carried to HEAD inside another agent's tree commit `39fa2540`; the measurements are here. `recover_facets_with_points` rebuilt the entire face set once per PENDING FACET inside every pass, so a pass cost O(facets × tets) in map inserts. Nothing about a facet's round changes the mesh unless it inserts a Steiner point, so the pass now carries one face set and refreshes it only after an insertion, and the round-cap trace reuses that set instead of building a second. The shape of the waste, measured on the vented enclosure: 188 facets over 1137 tets with ONE Steiner point inserted, so 188 full face maps were built per pass to observe a mesh that changed once. **No wall-clock claim**: the comparison run landed on a host swinging between load 44 and 692, which inflated fs-mesh conformance from 72 s to 1170 s in the same run, so the effect is recorded as the term removed and must be re-measured on a quiet host before any number is quoted. The full fs-mesh suite is green with the change and every corpus outcome is unchanged. **Process note:** this is the third consecutive increment (`4836e81d`, `39fa2540`, and earlier `df959c71`) carried to HEAD by another agent's tree-wide commit while verification was still running, under a message describing someone else's work. The code was green each time, but a tree-wide sweep cannot know that; it is a standing hazard to HEAD and an owner decision, not something this lane can fix from inside.
