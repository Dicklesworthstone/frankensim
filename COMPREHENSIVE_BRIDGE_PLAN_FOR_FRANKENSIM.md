...
**A13. Export of a run retained under an older receipt schema refuses as "unknown run" (new, q61wp.63).** *State:* measured 2026-09-02 — the 00:40 binary (conduction receipt v4) refuses to export a run the 22:38 binary (v3) retained, with `cli-solve-unknown-run`; a fresh run exports. *Work:* name the real reason (`cli-solve-run-incompatible`, schemas and driver versions), define the compatibility rule (additive receipt bumps keep old runs exportable), fixture test, CONTRACT sentence. *Size:* M.
