...
[![Tests](https://img.shields.io/badge/tests-826%20tracked%20integration%20test%20files-1f883d)](#contracts-and-verification)
...
| Tracked crate integration-test files | 826 |
...
| Contracts | 166 of 166 `fs-*` crate directories have `CONTRACT.md` files |
...
| Tests | 826 Rust files under crate `tests/` directories in the checked inventory, in addition to inline unit tests; this is an inventory count, not a claim that every test is green on the current tree |
...
| L3 | Integrated workflow | 1 | Steady conduction FEM solve on solids carries an admitted end-to-end integration claim via scripts/e2e/cooling_01.sh and the Cooling 0.1 product workflow |
...
| `governance.capability-maturity` — Capability maturity registry and promotion events | L2 | `xtask` | Promoted to L2 under bead frankensim-4gh8t. Rehearsed clean promotion (thermal.conduction-solve L2 -> L3) and rehearsed blocked-promotion defect gate are both verified live in xtask. |
...
| `thermal.conduction-solve` — Steady conduction FEM solve on solids | L3 | `fs-conduction`, `fs-feec`, `fs-solid` | Promoted to L3 under bead frankensim-4gh8t via end-to-end acceptance lane scripts/e2e/cooling_01.sh and the Cooling 0.1 integrated product workflow. Boundary: L3 integration covers the tracked single-command pipeline and composable verbs through report and evidence packaging. It does NOT cover L4 validation against an external physical corpus (e04), cross-ISA bit-exactness outside declared platforms, or nonlinear material extrapolation. |
...
The tree currently has 166 `CONTRACT.md` files for 166 `fs-*` crate directories.
...
|-- crates/                            # 166 fs-* crates; selected entries shown below
