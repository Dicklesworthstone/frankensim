...
[![Crates](https://img.shields.io/badge/workspace-148%20native%20fs--%2A%20crates-0969da)](#implemented-workspace)
[![Contracts](https://img.shields.io/badge/contracts-149%20of%20149%20crates-8250df)](#contracts-and-verification)
[![Tests](https://img.shields.io/badge/tests-658%20tracked%20integration%20test%20files-1f883d)](#contracts-and-verification)
...
|-------------------------|-------|
| Native workspace `fs-*` crates | 148 |
| Standalone `fs-*` workspaces | 1 (`fs-wasm`) |
| Tracked `fs-*` crate directories | 149 |
| Tracked `CONTRACT.md` files | 149 of 149 |
| Tracked crate integration-test files | 658 |
| `fs-*` layer inventory | `UTIL=15`, `L0=7`, `L1=14`, `L2=18`, `L3=31`, `L4=32`, `L5=4`, `L6=27`, `TOOL=1` |
...
|------|------------------------|
| Workspace | Rust 2024 nightly Cargo workspace with 148 native `fs-*` workspace crates plus `xtask`; `fs-wasm` is a standalone nested workspace |
| Contracts | 149 of 149 `fs-*` crate directories have `CONTRACT.md` files |
...
| L4 | Experimentally validated | 0 | The V&V corpus registry contains 19 Level-A thermal definitions/targets, a synthetic Level-B fixture, and a retained Level-C Martin-Moyce curve. Eighteen of the 19 Level-A rows have test-time executing-kernel bindings; the infinite-parallel-plate view-factor row remains reference-only because the current constructor stores its limiting value rather than computing it. No binding persists a corpus receipt or machine fingerprint; the experimental row also lacks original raw data, metrology, replayable digitization lineage, resolved redistribution authority, and a defensible scalar envelope. Every row is capped `Estimated`, so no L4 claim exists |
...
The tree currently has 149 `CONTRACT.md` files for 149 `fs-*` crate directories.
...
|-- crates/                            # 149 fs-* crates; selected entries shown below
