...
[![Crates](https://img.shields.io/badge/workspace-163%20native%20fs--%2A%20crates-0969da)](#implemented-workspace)
[![Contracts](https://img.shields.io/badge/contracts-182%20of%20183%20tracked%20crates-8250df)](#contracts-and-verification)
[![Tests](https://img.shields.io/badge/tests-878%20tracked%20integration%20test%20files-1f883d)](#contracts-and-verification)
...
The tree contains 183 `fs-*` crate directories: 163 in the native Cargo workspace
and 20 standalone workspaces listed below. They include numerical kernels,
geometry representations, meshing, physics, solvers, adjoints, optimization,
imaging, evidence, packaging and ledger implementations, together with their
contracts and tests.
...
`cargo run -p xtask -- check-docs`. The inventory includes committed paths;
the check also checks their presence in the current index. Staged removals can
therefore block this check until their inventory change lands. An RCH or archive source snapshot
...
|-------------------------|-------|
| Native workspace `fs-*` crates | 163 |
...
| Tracked `fs-*` crate directories | 183 |
| Tracked `CONTRACT.md` files | 182 of 183 |
| Tracked crate integration-test files | 878 |
| `fs-*` layer inventory | `UTIL=15`, `L0=7`, `L1=14`, `L2=22`, `L3=41`, `L4=33`, `L5=4`, `L6=46`, `TOOL=1` |
<!-- END GENERATED FRANKENSIM DOC FACTS -->

The CLI integrates bounded thermal/airflow solves and a normalized thermal
radius study with ledger, report and package outputs. It is still a source
workspace: no general CAD/CAE application or crates.io release is claimed.
...
|------|------------------------|
| Workspace | Rust 2024 nightly Cargo workspace with 163 native `fs-*` workspace crates plus `xtask`; 20 nested workspaces are listed above |
| Contracts | The committed inventory contains 182 contracts for 183 crates; the new `fs-scene/CONTRACT.md` is awaiting inclusion in the committed inventory |
...
| Tests | 878 Rust files under crate `tests/` directories in the committed inventory, in addition to inline unit tests; this count does not imply that every test is green |
...
| L4 | Experimentally validated | 0 | The V&V corpus registry contains 20 Level-A thermal definitions/targets, five thermal Level-B cross-code cases, and four retained Level-C published-experiment records (Martin–Moyce plus three further digitized curves). Eighteen of the 20 Level-A rows have test-time executing-kernel bindings; the infinite-parallel-plate view-factor row remains reference-only because the current constructor stores its limiting value rather than computing it. No binding persists a corpus receipt or machine fingerprint; the experimental row also lacks original raw data, metrology, replayable digitization lineage, resolved redistribution authority, and a defensible scalar envelope. Every row is capped `Estimated`, so no L4 claim exists |
...
| `governance.capability-maturity` — Capability maturity registry and promotion events | L2 | `xtask` | L2 structural maturity evidence is live in xtask. L3+ now requires a cited lane plus a retained receipt that records the capability stage as executed at a HEAD no older than that lane. No capability currently meets that bar. |
...
| `frankensim [--json] study --resume <study-id> <ledger.db> [--budget N]` | Replay the retained prefix and continue with its original wall/iteration budget; budget exhaustion and cancellation remain explicit partial outcomes |
| `frankensim [--json] report <study-id> <ledger.db>` | Export the retained HTML/SVG and JSON study report, preserving conflicting destination files |
| `frankensim [--json] package <study-id> <ledger.db>` | Re-check and export the retained package; structural verification does not upgrade its Estimated numerical claims |
...
The executable study fixture is
...
|---------------|-------|---------|------------------------------|
| `.fsim` project schema | `fs-project` | 4 | migrates, with a receipt |
...
| Euler artifact owner and schema-routing matrix | `fs-euler-disc-e2e` | 1 | no predecessor yet; every other version refuses |
| Euler claim and minimum-evidence policy | `fs-euler-disc-e2e` | 1 | no predecessor yet; unknown versions refuse |
| Euler structural checker and local evidence-reference assessment protocol | `fs-euler-disc-e2e` | 1 | no predecessor; the receipt decoder and complete canonical assessment-log reader refuse mismatches, while other bytes are identity preimages |
...
| Source manifest + SPDX projection | `xtask` | `frankensim-source-manifest-v1` | no predecessor yet |
| Capability-maturity registry | `xtask` | `frankensim-capability-maturity-v1` | no predecessor yet |

Five frozen schemas carry explicit lockstep obligations:

| Frozen schema | Lockstep constants and recorded values |
|---------------|----------------------------------------|
| Evidence-package format | `CHECKER_SUPPORTED_PACKAGE_FORMAT=9`, `CHECKER_DECISION_IDENTITY_VERSION=9`, `SUPPORTED_PACKAGE_FORMAT=9` |
| Euler scientific Context and composite contract | `VV_SCHEMA_VERSION=3`, `VV_ARTIFACT_FAMILY="org.frankensim.fs-evidence.vv-artifact.v3"`, `AUTHORITY_ALGEBRA_VERSION=2`, `EXPERIMENT_CAMPAIGN_SCHEMA_VERSION_V1=1` |
...
the first bump until the record declares how the predecessor is handled. The seven
such promises are `euler.scientific-contract`, `euler.owner-matrix`,
`euler.claim-policy`, `euler.protocol`, `matdb.pack`, `sbom.source_manifest`, and
`maturity.registry`.
...
| `fs-iga`, `fs-vpm`, `fs-fmm`, `fs-lbm`, `fs-flux`, `fs-bem` | Physics scaffolds at their actual scope: `fs-iga` is a 1-D clamped B-spline Poisson fixture (Kirchhoff–Love shells retired from v1, deferred epic frankensim-rc-root-q61wp.40); `fs-vpm` is a 2-D direct O(N²) point-vortex core and `fs-fmm` a uniform-depth Laplace FMM (FMM-accelerated VPM retired from v1, epic .43); `fs-lbm` is BGK D2Q9/D3Q19 with a thermal port and no turbulence model (turbulence retired from v1, epic .41; the `fs-scenario` RANS rung is a 1-D channel fixture); `fs-flux` is 2-D H(div)-DG incompressible flow on triangle fixtures; compressible flow has no code (epic .42); `fs-bem` is a 2-D panel/wake method plus 3-D non-lifting FMM panels and Helmholtz radiation |
...
|---------|-----------------|----------------|
| `fs-wasm::flagships` | Source for browser-facing `run_ornithoid`, `run_vessel`, and `run_frame` pipelines | The source composes reduced flagship workflows and builds again since the safe-step propagation (f85xj.2.39), but the capability registry holds browser flagships at L1 with two campaign tests still red from an earlier code-first slice; the internal "Tier V" campaign label is not an L5 maturity claim |
...
| Study the Euler-disc scientific boundary | `crates/fs-euler-disc-e2e/CONTRACT.md`, then its `contract` and `protocol` modules |

## Contracts and Verification

The committed inventory contains 182 `CONTRACT.md` files for 183 `fs-*` crate
directories. The `fs-scene` contract has been added to the working tree; its
inventory inclusion remains part of landing that file.
...
|-- crates/                            # 183 fs-* crates; selected entries shown below
...
|   |-- fs-euler-disc-e2e/             # Euler-disc scientific contract and no-claim boundary
...
| Convection correlations | `fs-convection` implements 12 sourced analytic or empirical relations with machine-readable domains and structured refusal, plus a tested Robin seam into `fs-conduction`. One card manually encodes a narrow Shah-London developing-rectangular Table 52 slice and labels its lower-`Gz` limit bridge as an engineering interpolation. Its empirical discrepancy bands are declared engineering allowances rather than source-published confidence intervals; a broader source-table corpus, automatic model selection/blending, interrupted-fin and array-interference cards, conjugate CFD, experimental validation, and a registered maturity claim remain absent |
...
| Production validation corpus | Partial, not production-ready. `fs-vvreg` registers 20 Level-A thermal formula/order definitions, five thermal Level-B cross-code cases, and four retained Level-C published-experiment records (the Martin-Moyce 1952 digitized curve plus three further published records). `fs-conduction` resolves eight analytic rows, including pole-free spherical-shell and matching-P1 two-slab contact cases, plus seven MMS rows (four P1 primal, one P1 adjoint, one P2 primal, and one P2 adjoint) while executing its tests; both P2 ladders use the high-order FEEC tetrahedral kernel. The infinite-parallel-plate view-factor row remains reference-only because the current constructor stores `F12 = F21 = 1` literally rather than computing visibility from geometry. `fs-convection` resolves the two circular-duct Nusselt limits; `fs-time` resolves the normalized small-Biot lumped decay through dense and operator-backed first-order generalized-alpha paths. The complete crosswalks therefore bind 18 of 20 Level-A rows and preserve the view-factor row as an explicit gap. The fourth primal P1 MMS binding combines a rotated orthotropic frame, three distinct linear `k_i(T)` laws, the full chain-rule manufactured source, and nonlinear iteration; it is observed-order evidence on the declared mesh ladder, not a universal convergence proof. The P1 heat-adjoint binding solves a quartic manufactured dual through `fs-adjoint`, checks the discrete primal/dual identity on every rung, and gates its L2 order; it is not a goal-oriented error bound. The P2 primal binding runs the existing `fs_feec` order-2 simplex stiffness/load/L2 path for a constant-isotropic homogeneous-Dirichlet fixture. The P2 heat-adjoint binding solves the manufactured dual through the same high-order operator and `fs-adjoint`, then checks the discrete primal/dual identity on every rung. Neither P2 binding turns the P1 `fs-conduction` material/boundary frontend into a general P2 thermal frontend, supplies a P2 design-gradient pullback, or establishes a DWR estimate. These are test-time crosswalks, not retained corpus receipts: solver/model envelopes remain distinct from the catalog's formula-reproduction tolerances, no ladder or machine fingerprint is persisted into `fs-vvreg`, and registry queries remain `Estimated` with numerical `NoClaim`. The contact binding is limited to exact duplicated matching-P1 traces and retains an ordered material-card/property receipt in the solve report; it has no nonmatching/mortar projection, variable contact law, scenario-object auto-lowering, or retained registry authority. The radiation battery proves gray-diffuse algebra for an admitted matrix, an unequal-area enclosure closure, and a coupled closed-form heat balance; it does not implement a view-factor formula, establish a geometry/QMC generator, carry a radiation-dominated retained corpus case, provide nonlinear uncertainty authority, or confer L4 validation. The lumped binding proves only the normalized scalar ODE under the catalog's applicability context, not a spatial transient-conduction model. The external Level-C row is derived-only and still lacks original raw records, instrument/calibration/placement authority, acquisition conditions/date, replayable digitization lineage, resolved redistribution terms, and a defensible scalar acceptance envelope. No capability may infer L4 from registration or these test bindings alone |
