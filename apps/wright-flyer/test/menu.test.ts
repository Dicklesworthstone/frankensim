...
import { DEFAULT_SELECTION, KEY_LINES, MODE_CARDS, menuQuery } from "../src/menu.ts";
...
test("selection to query, exact strings", () => {
  ...
  assert.equal(
    ...
    "?sim=1&mode=human&site=huffman&assist=1",
  );
...
test("assist never leaks into non-human modes", () => {
  assert.equal(
