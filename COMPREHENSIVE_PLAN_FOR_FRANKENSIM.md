...
**P1 — Pure memory-safe Rust, Franken-only dependencies.** The runtime dependency set is exactly: `std`, asupersync, FrankenSQLite, FrankenNumpy, FrankenTorch, FrankenScipy, FrankenPandas, FrankenNetworkx. No BLAS, no LAPACK, no C, no C++, no FFI. `unsafe` is permitted only inside a small set of audited leaf modules (SIMD microkernels, arena allocators, memory mapping), each under 300 lines and behind a safe façade. The release target is exhaustive testing, including an in-house model-checking harness that enumerates interleavings of the concurrency primitives it faithfully represents. **Current-v0 no-claim:** that target is not yet met for FrankenSim's `TilePool` mutex/condvar protocol, arena leases, parked crew, or fault-drain paths. Their present batteries are bounded seeded/runtime tests, and asupersync's model checks do not cover those FrankenSim protocols.
...
- **asupersync** — Demands: structured-concurrency scopes with sub-millisecond latency-to-cancel at tile granularity, cancellation trees deep enough for speculative races over solver hierarchies, and stable semantics under the G4 chaos harness. Intended upstream returns: an executor model-checking harness, cancellation-latency benchmarks, and the two-lane scheduler as a reusable pattern. **Current-v0 no-claim:** asupersync's own model-checking evidence covers only the protocols represented in its models; it does not cover FrankenSim's `TilePool`, arena-lease, parked-crew, or injected-fault protocols.
...
- **G4 — Chaos and cancellation storms.** Kill random scopes at random times during real solves; assert zero leaks via arena accounting, no deadlocks, ledger consistency, and that pause–serialize–resume reproduces the uninterrupted trajectory in deterministic mode. Fault-inject tiles, poison allocators, torture the two-lane executor with adversarial preemption; run a faithful model checker over the executor's core protocols. Cancellation-correctness (P7) is a *claim*; G4 is the proof engine. **Current-v0 status:** the `TilePool` mutex/condvar, arena-lease, parked-crew, poison, and injected-fault batteries are bounded seeded/runtime tests, not exhaustive schedule enumeration. Their retained seeds make reached cases replayable; they do not make all interleavings covered. asupersync's model checks cover its own modeled protocols only and do not establish this FrankenSim evidence. The exhaustive model-checking target remains open until a faithful model covers these protocols and their drain/finalize interactions.
